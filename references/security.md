...
- **审计哈希链**: 每条审计记录含 `prev_hash`(链向前一条)与 `hash`(本条内容 SHA-256),形成链式结构。`store.verify_audit_chain()` 可检测任何就地篡改/删除/插入——任一改动都会让断链处或内容-hash 不符处报错。
  - **局限**: 链头(首条)没有可信锚点,理论上攻击者可整体替换整条链(需同时重算所有 hash)。这是**防御纵深**(让单点篡改变得可检测、抬高成本),不是密码学防伪。真正锚点需把链头 hash 定期写到独立可信位置(如另一个服务、纸质记录)。
